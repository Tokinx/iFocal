<template>
  <div class="rounded-xl bg-white shadow-xs relative z-0">
    <div class="sticky top-0 z-10 bg-slate-50">
      <div class="flex items-center justify-between gap-2 p-1.5 bg-slate-50/95 border backdrop-blur-sm rounded-t-xl"
        :class="card.collapsed ? 'rounded-b-xl' : 'border-b border-slate-200/75'">
        <div class="flex items-center gap-1.5 min-w-0">
          <Tooltip>
            <TooltipTrigger as-child>
              <span class="ifocal-drag-handle cursor-grab text-slate-400 hover:text-slate-600">
          ...
          <span v-else class="truncate text-sm font-medium text-slate-700">{{ title }}</span>
        </div>
        <div class="flex items-center gap-0.5">
          <Tooltip v-if="!card.collapsed && (runtime.result || runtime.error)">
            <TooltipTrigger as-child>
              <Button variant="ghost" size="icon" class="h-6 w-6 rounded-lg text-slate-500 hover:bg-slate-100"
          ...
          <Tooltip>
            <TooltipTrigger as-child>
              <Button variant="ghost" size="icon"
                class="h-6 w-6 rounded-lg text-slate-500 hover:text-red-500 hover:bg-red-50"
    ...
    <div v-if="!card.collapsed"
      class="p-2.5 text-sm leading-relaxed rounded-b-xl border border-t-0 border-slate-200/75">
      <div v-if="runtime.loading" class="flex items-center text-xs gap-2 text-slate-400">
        <span class="shimmer-text">正在翻译...</span>
      </div>
      <div v-else-if="runtime.error" class="text-xs text-red-500 whitespace-pre-wrap">
        {{ runtime.error }}
      </div>
      <div v-else-if="runtime.result" class="whitespace-pre-wrap text-slate-800">
        {{ runtime.result }}
      </div>
      ...
      <div v-if="runtime.durationMs > 0 && !runtime.loading" class="mt-1 text-[11px] text-slate-400">
