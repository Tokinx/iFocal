<template>
  <aside :class="['shrink-0 duration-200', showAssistantPanel ? 'w-68' : 'w-none']">
    <div class="flex h-full min-h-0 flex-row gap-2">
      <div class="flex flex-col gap-2 shrink-0">
        <Button variant="outline"
          class="rounded-1xl justify-center h-9 w-9 gap-2 bg-white hover:bg-white/60 shadow-xs"
          :class="activeRouteName === 'assistant' ? '!bg-amber-700 !text-white' : ''"
          title="助手"
          @click="$emit('navigate', 'assistant')">
          <Icon icon="proicons:sparkle-2" class="h-4 w-4" />
        </Button>
        <Button variant="outline"
          class="rounded-1xl justify-center h-9 w-9 gap-2 bg-white hover:bg-white/60 shadow-xs"
          :class="activeRouteName === 'translate' ? '!bg-amber-700 !text-white' : ''"
          title="翻译"
        ...
        <Button variant="outline"
          class="rounded-1xl justify-center h-9 w-9 gap-2 bg-white hover:bg-white/60 shadow-xs"
          :class="activeRouteName === 'settings' ? '!bg-amber-700 !text-white' : ''"
          title="设置"
          @click="$emit('navigate', 'settings')">
          <Icon icon="ri:settings-4-line" class="h-4 w-4" />
        </Button>
      </div>
      <div v-if="showAssistantPanel" class="w-0 flex-1 flex h-full min-h-0 flex-col gap-2">
        <div class="p-3 rounded-1xl space-y-1 bg-white border border-olive-300/60 shadow-xs">
          <div v-for="item in tasks" :key="item.id" role="button" tabindex="0"
            ...
            :class="activeRouteName === 'assistant' && activeAssistantId === item.id ? '!bg-amber-700/90 !text-white' : 'text-foreground'"
...
// 只有助手页面展开右侧助手/会话面板；翻译、设置页面只显示左侧图标导航
const showAssistantPanel = computed(() => props.activeRouteName === 'assistant')
