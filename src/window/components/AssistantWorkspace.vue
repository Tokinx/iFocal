  ...
  <div class="relative h-full min-h-0 flex-1">
    ...
    >
      <header class="flex items-center justify-end absolute top-0 left-0 right-0 p-4 z-10">
        <LanguageSelect
          :current-lang-label="ctx.currentLangLabel"
          :current-target-lang="ctx.targetLang"
          :supported-languages="ctx.supportedLanguages"
          :bg-class="ctx.bgClass"
          :blur-class="ctx.blurClass"
        ...
        <template v-for="(message, idx) in ctx.messages" :key="idx">
          <div v-if="message.role === 'user'" class="flex justify-end">
            <div class="group relative max-w-[80%]">
              <div v-if="message.attachments && message.attachments.length > 0" :class="['space-y-2', { 'mb-2': message.content }]">
              ...
              <div
                ...
                v-html="ctx.renderMarkdownSafe(message.content)"
            ...
            <div class="flex items-center justify-between">
              ...
              <div class="flex items-center gap-1">
                <Button
                  variant="ghost"
                  size="icon"
                  class="h-6 w-6 opacity-0 group-hover:opacity-100 transition-opacity text-olive-400"
                  title="复制"
            ...
            <div class="w-full">
              ...
              <div v-else-if="message.content">
                <template v-if="ctx.getParsed(message, idx).reasoning">
                  <template v-if="message.isStreaming && ctx.enableReasoning && !ctx.getParsed(message, idx).answer">
                    <div class="flex items-center">
                      <Button variant="ghost" size="xs" class="h-6 p-0 text-xs gap-1">
                        <Icon icon="ri:lightbulb-line" class="h-4 w-4 text-olive-400" />
                        <span class="text-xs text-olive-400 shimmer-text">
                          正在思考...
                        </span>
                      </Button>
                      <span class="ml-2 text-olive-400" v-if="ctx.getReasoningElapsedSeconds(message) > 0">
                  ...
                  <template v-else>
                    <div class="flex items-center">
                      <Button
                        variant="ghost"
                        size="xs"
                        class="group/inner h-6 p-0 text-xs gap-1 hover:bg-transparent"
                        @click="message.reasoningCollapsed = !message.reasoningCollapsed"
                      >
                        <div class="relative h-4 w-4 text-olive-400">
                          <Icon
                            icon="ri:lightbulb-line"
                            class="absolute left-0 top-0 opacity-100 group-hover/inner:opacity-0 !h-4 !w-4 transition-opacity"
                          />
                          <Icon
                            :icon="message.reasoningCollapsed ? 'ri:arrow-down-s-line' : 'ri:arrow-up-s-line'"
                      ...
                      <span class="ml-2 text-olive-50" v-if="ctx.getReasoningElapsedSeconds(message) > 0">
                        {{ ctx.getReasoningElapsedLabel(message) }}s
                      </span>
                    </div>
                    <div
                      v-if="!message.reasoningCollapsed"
                  ...
                  <div
                    class="prose prose-sm max-w-none !text-olive-800"
              ...
              <div v-else>
                ...
                <template v-else>
                  <div class="space-y-3">
                    <div class="h-3 w-2/3 bg-olive-50/20 animate-pulse" />
                    <div class="h-3 w-full bg-olive-50/20 animate-pulse" />
                    <div class="h-3 w-5/6 bg-olive-50/20 animate-pulse" />
                  </div>
                </template>
              </div>
            </div>
      ...
      <footer ref="footerEl" class="absolute left-0 right-0 bottom-0 p-4">
        <ChatInput
          ref="chatInputRef"
          :model-value="ctx.text"
          :sending="ctx.isBusy"
          :enable-streaming="ctx.enableStreaming"
          :enable-reasoning="ctx.enableReasoning"
          :reasoning-effort="ctx.reasoningEffort"
          :enable-context="ctx.enableContext"
          :enable-file-upload="ctx.enableFileUpload"
          :auto-paste-global-assistant="ctx.autoPasteGlobalAssistant"
          :bg-class="ctx.bgClass"
          :blur-class="ctx.blurClass"
          :current-model-name="ctx.currentModelName"
          :grouped-models="ctx.groupedModels"
          :selected-pair-key="ctx.selectedPairKey"
          @update:modelValue="ctx.updateText"
          @selectModel="ctx.selectModel"
          @send="ctx.handleSend"
          @stop="ctx.stopGenerating"
          @toggleStreaming="ctx.toggleStreaming"
          @toggleReasoning="ctx.toggleReasoning"
          @changeReasoningEffort="ctx.changeReasoningEffort"
          @toggleContext="ctx.toggleContext"
          @toggleClipboardListening="ctx.toggleClipboardListening"
          @toggleFileUpload="ctx.toggleFileUpload"
    ...
    <Button
      v-if="ctx.showScrollToBottomButton"
      variant="outline"
      size="icon"
      ...
      title="滚动到底部"
      @click="ctx.handleScrollToBottomClick"
    >
